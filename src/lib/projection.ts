...
import { Person, Contract, Asset, Liability, HouseholdConfig, Scenario } from '@/lib/types';

export interface ProjectionYear {
  year: number;
  income: number;
  expenses: number;        // base expenses + liability payments + study costs
  premiums: number;        // insurance premiums
  netCashflow: number;     // income - expenses - premiums
  investReturn: number;    // return on existing net worth
  netWorth: number;
  lifePhase: 'work' | 'partial-retirement' | 'retirement';
}
...
export function computeProjection(
  ...
  for (let i = 0; i <= PROJECTION_YEARS; i++) {
    ...
    // Life phase
    let lifePhase: ProjectionYear['lifePhase'] = 'work';
    if (adults.length > 0) {
      if (retiredCount === adults.length) lifePhase = 'retirement';
      else if (retiredCount > 0)          lifePhase = 'partial-retirement';
    }
